...
def explain_market_gemini(api_key, signal, fear, greed, neutral, vix, pcr, articles, query):
    """Generate AI analysis using Gemini - NO CACHING to avoid stale errors"""
    if not api_key:
...
with st.sidebar:
    ...
    # Show API status with better visibility
...
if st.button("🔄 Analyze Market Sentiment", type="primary", use_container_width=True):
    ...
    with st.spinner(f"🔍 Analyzing '{search_query}'..."):
        ...
        # AI explanation (NO CACHE - fresh call every time)
...
st.caption("Built with Streamlit • FinBERT • Gemini AI • GNews • NewsAPI • Finnhub")    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 429:
            st.warning("⚠️ Finnhub API rate limit reached")
        return []
    except Exception as e:
